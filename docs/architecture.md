...
| 항목 | 내용 |
|------|------|
| State | `NewsletterWorkflowState` (`ipn_agent.orchestrator.state`) |
...
| Node | 역할 | 구현 |
|------|------|------|
| `editor_prepare` | approved 경로 검증·입력 기사 count | `ipn_agent.orchestrator.editor.prepare_newsletter_context()` |
| `editor_generate` | analysis → standards_linker → hitl → editor | `ipn_agent.orchestrator.editor.generate_newsletter_draft()` |
| `editor_quality_check` | draft 품질·파일 존재 확인 | `ipn_agent.orchestrator.editor.refine_newsletter_draft()` |
| `draft_created` | run 완료 마킹 | — |

Editor 핵심 로직은 **`ipn_agent.orchestrator.editor`**에 분리되고, 실제 노드 함수(`analysis_node` 등)는 `ipn_agent.orchestrator.newsletter`에 있다. Orchestrator node가 이를 직접 호출한다.
...
| 경로 | 용도 | LangGraph |
|------|------|-----------|
| **`newsletter_orchestrator.py`** | Streamlit 기본 · CLI · **유일한 진입점** | ✅ 단일 Graph |
| `research_review_agent.py` | 레거시 Collect 전용 subprocess (fetch→expansion→radar→review) | ❌ subprocess |

> v0.8에서 `pipeline_graph.py`(deprecated wrapper)와 `newsletter_agent_skeleton.py`(Chat UI용 Editor subgraph, `langgraph.json` 포함)를 삭제했습니다. Chat UI는 최종적으로 미구현 상태로 종료됐습니다.

## threshold HITL (`ipn_agent.review.hitl`)
...
## 기발행 차단 (`ipn_agent.registry.published`)
...
## Review 메타 정규화 (`ipn_agent.review.metadata`)
...
| Editor 로직 | `ipn_agent.orchestrator.editor` (함수) + Orchestrator editor node |
